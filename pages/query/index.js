                                                            ...
                                                            <ListItem key={scaleName} sx={{ display: 'block' }}>
                                                                ...
                                                                <Box sx={{ display: 'flex', gap: 2, mt: 0.5 }}>
                                                                    {CAGED_SHAPES.map(shape => (
                                                                        <Link
                                                                            key={shape}
                                                                            href={`/query/scale_${slugify(scaleName)}_matches_chord_${slugify(chordData.name)}_in_${keySlug}_key_and_${shape.toLowerCase()}_shape`}
                                                                            passHref
                                                                        >
                                                            ...
                                                            <ListItem key={arpName} sx={{ display: 'block' }}>
                                                                ...
                                                                <Box sx={{ display: 'flex', gap: 2, mt: 0.5 }}>
                                                                    {CAGED_SHAPES.map(shape => (
                                                                        <Link
                                                                            key={shape}
                                                                            href={`/query/arpeggio_${slugify(arpName)}_matches_chord_${slugify(chordData.name)}_in_${keySlug}_key_and_${shape.toLowerCase()}_shape`}
                                                                            passHref
                                                                        >
                                                                            <Typography variant="caption" sx={{ color: 'secondary.main', cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}>
                                                                                {shape} Shape
